...
func main() {
	...
	for pageScanner.Scan() {
		...
		for i := 0; i < quantity; i++ {
			// slice tricks time
			if len(stacks[fromStackIndex]) > 0 {
				var pop rune
				pop, stacks[fromStackIndex] = stacks[fromStackIndex][len(stacks[fromStackIndex])-1], stacks[fromStackIndex][:len(stacks[fromStackIndex])-1]

				pop = stacks[fromStackIndex][len(stacks[fromStackIndex])-1]
				stacks[fromStackIndex] = stacks[fromStackIndex][:len(stacks[fromStackIndex])-1]
